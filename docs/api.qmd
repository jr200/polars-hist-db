...
|-------|------|---------|-------------|
| `database` | `str | None` | `None` | Optional database/schema name |
| `hostname` | `str` | — | Database host |
| `backend` | `str` | `"mariadb"` | Database backend |
| `port` | `int` | `3306` (MariaDB), `5432` (XTDB) | Connection port |
| `adbc_port` | `int | None` | `9832` (XTDB), otherwise `None` | FlightSQL/ADBC ingest port |
| `username` | `str` | — | Database user |
| `password` | `str` | — | Database password |
| `max_rows_per_insert` | `int | None` | `10000` (XTDB), otherwise `None` | Chunk size for XTDB inserts |
...
Backend adapters provide the storage boundary used by the core project for
MariaDB and XTDB.
...
- `MariaDbBackend` — complete adapter with default support and SQLAlchemy
  compatibility.
- `XtdbBackend` — XTDB adapter with SQL/pgwire + ADBC/FlightSQL paths for
  reads, inserts, and temporal upserts.
